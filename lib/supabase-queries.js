...
export async function getGaleri() {
  if (!isSupabaseConfigured || !supabaseClient) {
    return fallbackGaleri.map(item => ({
      ...item,
      foto: item.foto_url,
      fotos: [
        { id: '1', foto_url: item.foto_url },
        { id: '2', foto_url: 'https://picsum.photos/400/300?random=1' },
        { id: '3', foto_url: 'https://picsum.photos/400/300?random=2' }
      ]
    }));
  }
  try {
    ...
    if (error) throw error;
    return (data || []).map((item) => {
      const fotosSorted = (item.galeri_foto || []).sort((a, b) => a.urutan - b.urutan);
      return {
        ...item,
        foto: item.foto_cover || item.foto_url,
        fotos: fotosSorted,
      };
    });
  } catch {
...
export async function getLatestGaleriWithPhotos(limit = 4) {
  if (!isSupabaseConfigured || !supabaseClient) {
    return fallbackGaleri.slice(0, limit).map(item => ({
      ...item,
      foto: item.foto_url,
      fotos: [
        { id: '1', foto_url: item.foto_url },
...
export async function getDesaInfo() {
  ...
  try {
    ...
    return {
      ...
      petaEmbedUrl: data.peta_embed_url || "",
    };
  } catch {
    return fallbackDesaInfo;
  }
